...
from dtos.blockade import Blockade, BlockadeSummary, InterceptionDetails


async def start_blockade(fleet_id: int, world_id: int, target_faction_ids: List[int]):
...
async def get_blockade(blockade_id: int) -> Optional[Blockade]:
    row = await db.fetchrow("""
        ...
        WHERE b.id = $1
    """, blockade_id)
    return Blockade.from_row(row) if row else None
...
async def get_blockades(faction_id=None, world_id=None) -> List[BlockadeSummary]:
    _BASE_QUERY = """
        SELECT
            b.id,
    ...
    return BlockadeSummary.from_rows(rows)
...
async def get_interception_details(fleet_id: int) -> Optional[InterceptionDetails]:
    row = await db.fetchrow("""
        SELECT COALESCE(f.name, 'Unit #' || f.faction_fleet_number) AS unit_name,
               COALESCE(fac.formal_name, fac.name) AS faction_name
        FROM fleets f
        JOIN factions fac ON f.faction_id = fac.id
        WHERE f.id = $1
    """, fleet_id)
    return InterceptionDetails.from_row(row) if row else None
