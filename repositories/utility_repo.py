...
async def get_user_access_row(user_id: int) -> Optional[User]:
    row = await db.fetchrow("SELECT id, access_level, badge_ids FROM users WHERE id = $1", user_id)
...
async def get_badge_by_id(badge_id: int) -> Optional[dict]:
    row = await db.fetchrow("SELECT id, name FROM badges WHERE id = $1", badge_id)
    return dict(row) if row else None


async def get_badge_by_name(name: str) -> Optional[dict]:
    row = await db.fetchrow("SELECT id, name FROM badges WHERE LOWER(name) = LOWER($1)", name)
    return dict(row) if row else None
...
async def fetch_all_rows_for_quoted_table(quoted_table_name: str) -> list[dict]:
    rows = await db.fetch(f'SELECT * FROM "{quoted_table_name}"')
    return [dict(r) for r in rows]
