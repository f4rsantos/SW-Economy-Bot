...
async def trades(interaction: discord.Interaction, faction: str):
    ...
    if outgoing:
        lines = [f"**ID {t.id}:** {handle_return(t.amount)} {t.resource_name} → {t.other_faction_name}{_format_world_route(t)}" for t in outgoing[:10]]
        embed.add_field(name=f"Outgoing ({len(outgoing)})", value="\n".join(lines), inline=False)
        if len(outgoing) > 10:
            embed.add_field(name="...", value=f"and {len(outgoing) - 10} more", inline=False)

    if incoming:
        lines = [f"**ID {t.id}:** {handle_return(t.amount)} {t.resource_name} from {t.other_faction_name}{_format_world_route(t)}" for t in incoming[:10]]
...
async def setup(bot):
    trades.autocomplete('faction')(faction_autocomplete)
